...
Create `/etc/systemd/system/firefleethq.service`:

```ini
[Unit]
Description=FireFleet HQ Gunicorn Application Server
After=network.target

[Service]
User=www-data
Group=www-data
WorkingDirectory=/path/to/firefleethq
Environment="PATH=/path/to/firefleethq/venv/bin"
Environment="FLASK_ENV=production"
ExecStart=/path/to/firefleethq/venv/bin/gunicorn \
    --worker-class gevent \
    --workers 4 \
    --bind 0.0.0.0:8000 \
    --timeout 30 \
    --access-logfile /var/log/firefleethq/access.log \
    --error-logfile /var/log/firefleethq/error.log \
    --log-level info \
    wsgi:application
...
sudo mkdir -p /var/log/firefleethq
sudo chown www-data:www-data /var/log/firefleethq

# Reload systemd
sudo systemctl daemon-reload

# Enable service
sudo systemctl enable firefleethq

# Start service
sudo systemctl start firefleethq

# Check status
sudo systemctl status firefleethq

# View logs
sudo journalctl -u firefleethq -f
```

### Option 5: Using Supervisor (Alternative to systemd)

Create `/etc/supervisor/conf.d/firefleethq.conf`:

```ini
[program:firefleethq]
command=/path/to/firefleethq/venv/bin/gunicorn --worker-class gevent --workers 4 --bind 0.0.0.0:8000 --timeout 30 wsgi:application
directory=/path/to/firefleethq
user=www-data
autostart=true
autorestart=true
redirect_stderr=true
stdout_logfile=/var/log/firefleethq/gunicorn.log
environment=FLASK_ENV=production
```
...
sudo supervisorctl start firefleethq
```
...
```bash
sudo cp nginx.conf.example /etc/nginx/sites-available/firefleethq
sudo nano /etc/nginx/sites-available/firefleethq
```

2. Update the following in the config file:
   - Replace `your-domain.com` with your actual domain
   - Update SSL certificate paths
   - Update static file path (`/path/to/firefleethq/app/static`)
   - Update upstream server if using different port

3. Enable the site:
```bash
sudo ln -s /etc/nginx/sites-available/firefleethq /etc/nginx/sites-enabled/
sudo nginx -t  # Test configuration
sudo systemctl reload nginx
```

Or create `/etc/nginx/sites-available/firefleethq` manually:

```nginx
upstream firefleethq {
...
server {
    ...
    # Logging
    access_log /var/log/nginx/firefleethq_access.log;
    error_log /var/log/nginx/firefleethq_error.log;

    # Static files (if serving directly from Nginx)
    location /static {
        alias /path/to/firefleethq/app/static;
        expires 30d;
...
- **Logs**: Check `/var/log/firefleethq/` or journalctl for errors
